...
export function ConsultationPage() {
  const { id } = useParams()
  const { appointments, finishAppointment } = useApp()
  ...
  const finish = async () => {
    ...
    navigate(`/citas/${appointment.id}/documentos`)
  }

  return (
    <div className="consultation-page">
      <div className="prototype-bar">Prototipo académico — No es un servicio médico real</div>
      <header className="consultation-header"><div><strong>Videoconsulta con {appointment.doctorName}</strong><span>{appointment.patientName} · {appointment.specialty}</span></div><button className="button button-danger" onClick={finish}><PhoneOff />Finalizar consulta</button></header>
      <iframe
